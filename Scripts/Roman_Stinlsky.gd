	...
	if easy_doppel == 0:
		...
		match doppel_random:
			4: # Small nose
				...
				var random = randi_range(0,4) # For random entry request
				match random:
					0:
						if Global.language == SPANISH_VALUE:
			...
			5: # Fake nose
				...
				var random = randi_range(0,6) # For random entry request
				match random:
					0:
						if Global.language == SPANISH_VALUE:
							entry_request_reason = ENTRY_REASON_DOPPEL_02_SECOND_SPANISH
			...
			6: # Third eye
				...
				var random = randi_range(0,2) # For random ID
				match random:
					0:
		...
		match doppel_random:
			...
			1: # 2 eyebrows
				...
				eyes_sprite.set_texture(eyes_doppel_01)
				var random = randi_range(0,4) # For random entry request
				var random_02 = randi_range(0,5) # For random date ID
				match random_02:
					0:
