...
func _on_time_between_text_timeout()->void :
	if n_texts > 0:
		...
		if is_3312_active:
			...
			if door_access.visible and current_char != "Clownba":
				_stop_tutorial()
			else :
...
func _to_game_over()->void :
	Global.current_day_count += 1
	if game_over.is_visible_in_tree():
		Global.current_day_count = 0
		sfx_background.stop()
		black.visible = true
		_play_sfx(49)
		await get_tree().create_timer(TWEEN_TIME * 2).timeout
	if Global.record_day_count < Global.current_day_count:
		Global.record_day_count = Global.current_day_count
	Global.save_record()
	await get_tree().create_timer(TWEEN_TIME).timeout
	Global.scene = GAME_OVER_SCENE
	get_tree().change_scene_to_file(LOADING_DIR)
