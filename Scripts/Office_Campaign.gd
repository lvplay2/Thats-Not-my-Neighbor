...
@onready var x: Sprite2D = $Background / Wall / x
@onready var x_2: Sprite2D = $Background / Wall / x2
@onready var x_3: Sprite2D = $Background / Wall / x3
@onready var x_4: Sprite2D = $Background / Wall / x4
@onready var x_5: Sprite2D = $Background / Wall / x5
@onready var x_6: Sprite2D = $Background / Wall / x6
@onready var ddd: Sprite2D = $Camera / ID / DDD
@onready var ddd_2: Sprite2D = $Camera / ID / DDD2
@onready var ddd_3: Sprite2D = $Camera / ID / DDD3
@onready var ddd_4: Sprite2D = $Camera / ID / DDD4
@onready var ddd_logo_2: Sprite2D = $Camera / Entry_Request / DDD_Logo2
@onready var ddd_logo_3: Sprite2D = $Camera / Entry_Request / DDD_Logo3
@onready var ddd_logo_4: Sprite2D = $Camera / Entry_Request / DDD_Logo4
...
func _process(_delta)->void :
	...
	if id_camera.visible or entry_request.visible or today_list.visible or checklist.visible or folder_open_A.visible or folder_open_B.visible or folder_open_C.visible:
...
func _neighbor_in()->void :
	if chars_level_list.is_empty() or Global.red_handed:
...
func _nothing_on_camera():
	return !is_phone_UI_active and !_is_folder_open() and ( !checklist.is_visible_in_tree() or checklist_minimize_flag) and !entry_request.is_visible_in_tree() and !id_camera.is_visible_in_tree() and !today_list.is_visible_in_tree()

func _nothing_on_camera_but_phone_ui():
	return !_is_folder_open() and ( !checklist.is_visible_in_tree() or checklist_minimize_flag) and !entry_request.is_visible_in_tree() and !id_camera.is_visible_in_tree() and !today_list.is_visible_in_tree()
...
func _call_event(number)->void :
	match number:
		...
		1152:
			if "Roman_Stilnsky" in Global.neighbors_alive_list and "Lois_Stilnsky" in Global.neighbors_alive_list:
			...
			elif "Roman_Stilnsky" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[0] in Global.char_at_home_list:
					_text_on_call_selection(1152, 4)
				else :
					_text_on_call_selection(1152, 0)
			elif "Lois_Stilnsky" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[1] in Global.char_at_home_list:
					_text_on_call_selection(1152, 5)
				else :
					_text_on_call_selection(1152, 0)
			else :
				_text_on_call_selection(1152, 0)
		6431:
			if "Margarette_Bubbles" in Global.neighbors_alive_list:
				...
				if Global.CHAR_LIST[2] in Global.char_out_home_list:
					...
					_text_on_call_selection(6431, 0)
		4258:
			if "Alf_Cappuccin" in Global.neighbors_alive_list and "Rafttelyn_Cappuccin" in Global.neighbors_alive_list:
			...
			elif "Alf_Cappuccin" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[3] in Global.char_at_home_list:
					_text_on_call_selection(4258, 4)
				else :
					_text_on_call_selection(4258, 0)
			elif "Rafttelyn_Cappuccin" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[7] in Global.char_at_home_list:
					_text_on_call_selection(4258, 5)
				else :
					_text_on_call_selection(4258, 0)
			else :
					_text_on_call_selection(4258, 0)
		6996:
			if "Elenois_Sverchzt" in Global.neighbors_alive_list and "Selenne_Sverchzt" in Global.neighbors_alive_list:
			...
			elif "Elenois_Sverchzt" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[4] in Global.char_at_home_list:
					_text_on_call_selection(6996, 4)
				else :
					_text_on_call_selection(6996, 0)
			elif "Selenne_Sverchzt" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[5] in Global.char_at_home_list:
					_text_on_call_selection(6996, 5)
				else :
					_text_on_call_selection(6996, 0)
			else :
				_text_on_call_selection(6996, 0)
		7332:
			if "Izaack_Gauss" in Global.neighbors_alive_list:
				...
				_text_on_call_selection(7332, 0)
		5123:
			if "Arnold_Schmicht" in Global.neighbors_alive_list and "Gloria_Schmicht" in Global.neighbors_alive_list:
			...
			elif "Arnold_Schmicht" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[8] in Global.char_at_home_list:
					_text_on_call_selection(5123, 4)
				else :
					_text_on_call_selection(5123, 0)
			elif "Gloria_Schmicht" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[9] in Global.char_at_home_list:
					_text_on_call_selection(5123, 5)
				else :
					_text_on_call_selection(5123, 0)
			else :
				_text_on_call_selection(5123, 0)
		2668:
			if "Robertosky_Peachman" in Global.neighbors_alive_list and "Albertosky_Peachman" in Global.neighbors_alive_list:
			...
			elif "Robertosky_Peachman" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[10] in Global.char_at_home_list:
					_text_on_call_selection(2668, 4)
				else :
					_text_on_call_selection(2668, 0)
			elif "Albertosky_Peachman" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[11] in Global.char_at_home_list:
					_text_on_call_selection(2668, 5)
				else :
					_text_on_call_selection(2668, 0)
			else :
				_text_on_call_selection(2668, 0)
		1425:
			if "Mia_Stone" in Global.neighbors_alive_list and "Dr_W_Afton" in Global.neighbors_alive_list:
			...
			elif "Mia_Stone" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[12] in Global.char_at_home_list:
					_text_on_call_selection(1425, 4)
				else :
					_text_on_call_selection(1425, 0)
			elif "Dr_W_Afton" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[13] in Global.char_at_home_list:
					_text_on_call_selection(1425, 5)
				else :
					_text_on_call_selection(1425, 0)
			else :
				_text_on_call_selection(1425, 0)
		5513:
			if "Angus_Ciprianni" in Global.neighbors_alive_list:
				...
				_text_on_call_selection(5513, 0)
		4122:
			if "Francis_Mosses" in Global.neighbors_alive_list:
				...
				_text_on_call_selection(4122, 0)
		1346:
			if "Anastacha_Mikaelys" in Global.neighbors_alive_list and "Nacha_Mikaelys" in Global.neighbors_alive_list:
			...
			elif "Anastacha_Mikaelys" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[16] in Global.char_at_home_list:
					_text_on_call_selection(1346, 4)
				else :
					_text_on_call_selection(1346, 0)
			elif "Nacha_Mikaelys" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[17] in Global.char_at_home_list:
					_text_on_call_selection(1346, 5)
				else :
					_text_on_call_selection(1346, 0)
			else :
				_text_on_call_selection(1346, 0)
		4242:
			if "Steven_Rudboys" in Global.neighbors_alive_list and "Mclooy_Rudboys" in Global.neighbors_alive_list:
			...
			elif "Steven_Rudboys" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[18] in Global.char_at_home_list:
					_text_on_call_selection(4242, 4)
				else :
					_text_on_call_selection(4242, 0)
			elif "Mclooy_Rudboys" in Global.neighbors_alive_list:
				if Global.CHAR_LIST[19] in Global.char_at_home_list:
					_text_on_call_selection(4242, 5)
				else :
					_text_on_call_selection(4242, 0)
			else :
				_text_on_call_selection(4242, 0)
	...
	match number:
		...
		1152:
			match n:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_1152_2_spanish = ["Hola, habla Roman Stilnsky.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_1152_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_1152_2_english = ["Hello, Roman Stilnsky speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_1152_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_1152_2_chinese = ["你好，我是Roman Stilnsky。", "我不在等待任何访客。"]
						text_on_call = list_text_call_1152_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_1152_2_spanish = ["Hola, habla Lois Stilnsky.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_1152_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_1152_2_english = ["Hello, Lois Stilnsky speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_1152_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_1152_2_chinese = ["你好，我是Lois Stilnsky。", "我不在等待任何访客。"]
						text_on_call = list_text_call_1152_2_chinese.duplicate()
		6431:
			match n:
				0:
		...
		4258:
			match n:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_4258_02_spanish = ["Hola, habla Alf.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_4258_02_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_4258_02_english = ["Hello, Alf speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_4258_02_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_4258_02_chinese = ["你好，我是Alf。", "我不在等待任何访客。"]
						text_on_call = list_text_call_4258_02_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_4258_02_spanish = ["Hola, habla Rafttellyn.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_4258_02_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_4258_02_english = ["Hello, Rafttellyn speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_4258_02_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_4258_02_chinese = ["你好，我是Rafttellyn。", "我不在等待任何访客。"]
						text_on_call = list_text_call_4258_02_chinese.duplicate()
		6996:
			match n:
				0:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_6996_2_spanish = ["Hola, habla Elenois Sverchzt.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_6996_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_6996_2_english = ["Hello, Elenois Sverchzt speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_6996_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_6996_2_chinese = ["你好，我是Elenois Sverchzt。", "我不在等待任何访客。"]
						text_on_call = list_text_call_6996_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_6996_2_spanish = ["Hola, habla Selenne Sverchzt.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_6996_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_6996_2_english = ["Hello, Selenne Sverchzt speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_6996_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_6996_2_chinese = ["你好，我是Selenne Sverchzt。", "我不在等待任何访客。"]
						text_on_call = list_text_call_6996_2_chinese.duplicate()
		7332:
			match n:
				0:
		...
		5123:
			match n:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_5123_2_spanish = ["Hola, habla Arnold Schmicht.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_5123_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_5123_2_english = ["Hello, Arnold Schmicht speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_5123_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_5123_2_chinese = ["你好，我是Arnold Schmicht。", "我不在等待任何访客。"]
						text_on_call = list_text_call_5123_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_5123_2_spanish = ["Hola, habla Gloria Schmicht.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_5123_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_5123_2_english = ["Hello, Gloria Schmicht speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_5123_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_5123_2_chinese = ["你好，我是Gloria Schmicht。", "我不在等待任何访客。"]
						text_on_call = list_text_call_5123_2_chinese.duplicate()
		2668:
			match n:
				0:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_2668_2_spanish = ["Hola, habla Robertsky Peachman.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_2668_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_2668_2_english = ["Hello, Robertsky Peachman speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_2668_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_2668_2_chinese = ["你好，我是Robertsky Peachman。", "我不在等待任何访客。"]
						text_on_call = list_text_call_2668_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_2668_2_spanish = ["Hola, habla Albertsky.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_2668_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_2668_2_english = ["Hello, Albertsky speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_2668_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_2668_2_chinese = ["你好，我是Albertsky。", "我不在等待任何访客。"]
						text_on_call = list_text_call_2668_2_chinese.duplicate()
		1425:
			match n:
				0:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_1425_2_spanish = ["Hola, habla Mia.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_1425_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_1425_2_english = ["Hello, Mia speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_1425_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_1425_2_chinese = ["你好，我是Mia。", "我不在等待任何访客。"]
						text_on_call = list_text_call_1425_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_2668_2_spanish = ["Hola, habla Dr. W. Afton.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_2668_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_2668_2_english = ["Hello, Dr. W. Afton speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_2668_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_2668_2_chinese = ["你好，我是Dr. W. Afton。", "我不在等待任何访客。"]
						text_on_call = list_text_call_2668_2_chinese.duplicate()
		5513:
			match n:
				0:
		...
		1346:
			match n:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_1346_2_spanish = ["Hola, habla Anastacha.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_1346_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_1346_2_english = ["Hello, Anastacha speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_1346_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_1346_2_chinese = ["你好，我是Anastacha。", "我不在等待任何访客。"]
						text_on_call = list_text_call_1346_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_1346_2_spanish = ["Hola, habla Nacha.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_1346_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_1346_2_english = ["Hello, Nacha speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_1346_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_1346_2_chinese = ["你好，我是Nacha。", "我不在等待任何访客。"]
						text_on_call = list_text_call_1346_2_chinese.duplicate()
		4242:
			match n:
				0:
				...
				4:
					if Global.language == SPANISH_VALUE:
						list_text_call_4242_2_spanish = ["Hola, habla Steven.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_4242_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_4242_2_english = ["Hello, Steven speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_4242_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_4242_2_chinese = ["你好，我是Steven。", "我不在等待任何访客。"]
						text_on_call = list_text_call_4242_2_chinese.duplicate()
				5:
					if Global.language == SPANISH_VALUE:
						list_text_call_4242_2_spanish = ["Hola, habla Mclooy Rudboys.", "No estoy esperando ninguna visita."]
						text_on_call = list_text_call_4242_2_spanish.duplicate()
					elif Global.language == ENGLISH_VALUE:
						list_text_call_4242_2_english = ["Hello, Mclooy Rudboys speaking.", "I'm not expecting any visitors today."]
						text_on_call = list_text_call_4242_2_english.duplicate()
					elif Global.language == CHINESE_VALUE:
						list_text_call_4242_2_chinese = ["你好，我是Mclooy Rudboys。", "我不在等待任何访客。"]
						text_on_call = list_text_call_4242_2_chinese.duplicate()
...
func _spawn_character(character)->void :
	...
	if is_doppel:
		...
		if randi_range(0, 1) == 0:
			...
			characters.get_child(0).make_doppelganger_campaign()
...
func _make_characters_list()->void :
	...
	match Global.current_campaign_day:
		0:
			normal_chars = 3
			doppel_chars = randi_range(1, 3)
			not_today_chars = 0
		1:
			normal_chars = randi_range(3, 4)
			doppel_chars = randi_range(3, 4)
			not_today_chars = 1
		2:
			normal_chars = randi_range(4, 5)
			doppel_chars = randi_range(3, 4)
			not_today_chars = 2
		3:
			normal_chars = randi_range(4, 5)
		...
		5:
			normal_chars = randi_range(4, 5)
			doppel_chars = randi_range(4, 5)
			not_today_chars = randi_range(2, 3)
		6:
			normal_chars = 7
			doppel_chars = randi_range(5, 6)
			not_today_chars = 2

	_set_chars_on_list(normal_chars, 0)
	Global.today_list = normal_char_level_list.duplicate()
	Global.char_at_home_list = Global.neighbors_alive_list.duplicate()
	Global.char_not_today_list = Global.neighbors_alive_list.duplicate()
	if "Lois_Stilnsky" in Global.char_not_today_list:
		Global.char_not_today_list.erase("Lois_Stilnsky")
	if "Rafttelyn_Cappuccin" in Global.char_not_today_list:
		Global.char_not_today_list.erase("Rafttelyn_Cappuccin")
	if "Anastacha_Mikaelys" in Global.char_not_today_list:
		Global.char_not_today_list.erase("Anastacha_Mikaelys")
	Global.char_out_home_list = Global.today_list
	Global.update_char_at_home_list()

	_set_chars_on_not_today_list(not_today_chars)
...
func _set_chars_on_list(type_amount, list)->void :
	var n_int = 0
	var char_total = len(Global.neighbors_alive_list) - 1
	var new_char = ""
	while n_int < type_amount:
		match list:
			0:
				if len(Global.neighbors_alive_list) < type_amount:
					type_amount = len(Global.neighbors_alive_list)
				if len(Global.neighbors_alive_list) != 0:
					if Global.current_campaign_day == 6 and "Dr_W_Afton" in Global.neighbors_alive_list and "Dr_W_Afton" not in normal_char_level_list:
						normal_char_level_list.append("Dr_W_Afton")
						n_int += 1
						Global.chars_on_level += 1
					new_char = Global.neighbors_alive_list[randi_range(0, char_total)]
			...
			2:
				if len(Global.neighbors_alive_list) != 0:
					...
					if random_list == 0:
						...
						new_char = Global.neighbors_alive_list[randi_range(0, len(Global.neighbors_alive_list) - 1)]
...
func _set_chars_on_not_today_list(number: int)->void :
...
func _set_info_folder_A_01()->void :
	...
	if Global.is_campaign:
		if "Roman_Stilnsky" not in Global.neighbors_alive_list:
			folder_a_name_label_01.text = " "
			folder_a_photo_01.set_texture(no_photo)
			folder_a_id_label_01.text = " "
			folder_a_occupation_label_01.text = " "
			folder_a_appearence_label_01.text = " "
		if "Lois_Stilnsky" not in Global.neighbors_alive_list:
...
func _set_info_folder_A_02()->void :
	...
	if Global.is_campaign:
		if "Robertosky_Peachman" not in Global.neighbors_alive_list:
			folder_a_name_label_01.text = " "
			folder_a_photo_01.set_texture(no_photo)
			folder_a_id_label_01.text = " "
			folder_a_occupation_label_01.text = " "
			folder_a_appearence_label_01.text = " "
		if "Albertosky_Peachman" not in Global.neighbors_alive_list:
			folder_a_name_label_02.text = " "
			folder_a_photo_02.set_texture(no_photo)
			folder_a_id_label_02.text = " "
			folder_a_occupation_label_02.text = " "
...
func _set_info_folder_A_03()->void :
	...
	if Global.is_campaign:
		if "Angus_Ciprianni" not in Global.neighbors_alive_list:
			folder_a_name_label_01.text = " "
			folder_a_photo_01.set_texture(no_photo)
			folder_a_id_label_01.text = " "
			folder_a_occupation_label_01.text = " "
...
func _set_info_folder_A_04()->void :
	...
	if Global.is_campaign:
		if "Elenois_Sverchzt" not in Global.neighbors_alive_list:
			folder_a_name_label_01.text = " "
			folder_a_photo_01.set_texture(no_photo)
			folder_a_id_label_01.text = " "
			folder_a_occupation_label_01.text = " "
			folder_a_appearence_label_01.text = " "
		if "Selenne_Sverchzt" not in Global.neighbors_alive_list:
			folder_a_name_label_02.text = " "
			folder_a_photo_02.set_texture(no_photo)
			folder_a_id_label_02.text = " "
			folder_a_occupation_label_02.text = " "
...
func _set_info_folder_B_01()->void :
	...
	if Global.is_campaign:
		if "Arnold_Schmicht" not in Global.neighbors_alive_list:
			folder_b_name_label_01.text = " "
			folder_b_photo_01.set_texture(no_photo)
			folder_b_id_label_01.text = " "
			folder_b_occupation_label_01.text = " "
			folder_b_appearence_label_01.text = " "
		if "Gloria_Schmicht" not in Global.neighbors_alive_list:
...
func _set_info_folder_B_02()->void :
	...
	if Global.is_campaign:
		if "Izaack_Gauss" not in Global.neighbors_alive_list:
			folder_b_name_label_01.text = " "
			folder_b_photo_01.set_texture(no_photo)
			folder_b_id_label_01.text = " "
			folder_b_occupation_label_01.text = " "
...
func _set_info_folder_B_03()->void :
	...
	if Global.is_campaign:
		if "Margarette_Bubbles" not in Global.neighbors_alive_list:
			folder_b_name_label_01.text = " "
			folder_b_photo_01.set_texture(no_photo)
			folder_b_id_label_01.text = " "
			folder_b_occupation_label_01.text = " "
...
func _set_info_folder_B_04()->void :
	...
	if Global.is_campaign:
		if "Nacha_Mikaelys" not in Global.neighbors_alive_list:
			folder_b_name_label_01.text = " "
			folder_b_photo_01.set_texture(no_photo)
			folder_b_id_label_01.text = " "
			folder_b_occupation_label_01.text = " "
			folder_b_appearence_label_01.text = " "
		if "Anastacha_Mikaelys" not in Global.neighbors_alive_list:
			folder_b_name_label_02.text = " "
			folder_b_photo_02.set_texture(no_photo)
			folder_b_id_label_02.text = " "
			folder_b_occupation_label_02.text = " "
...
func _set_info_folder_C_01()->void :
	...
	if Global.is_campaign:
		if "Mia_Stone" not in Global.neighbors_alive_list:
			folder_c_name_label_01.text = " "
			folder_c_photo_01.set_texture(no_photo)
			folder_c_id_label_01.text = " "
			folder_c_occupation_label_01.text = " "
			folder_c_appearence_label_01.text = " "
		if "Dr_W_Afton" not in Global.neighbors_alive_list:
...
func _set_info_folder_C_02()->void :
	...
	if Global.is_campaign:
		if "Francis_Mosses" not in Global.neighbors_alive_list:
			folder_c_name_label_01.text = " "
			folder_c_photo_01.set_texture(no_photo)
			folder_c_id_label_01.text = " "
			folder_c_occupation_label_01.text = " "
...
func _set_info_folder_C_03()->void :
	...
	if Global.is_campaign:
		if "Steven_Rudboys" not in Global.neighbors_alive_list:
			folder_c_name_label_01.text = " "
			folder_c_photo_01.set_texture(no_photo)
			folder_c_id_label_01.text = " "
			folder_c_occupation_label_01.text = " "
			folder_c_appearence_label_01.text = " "
		if "Mclooy_Rudboys" not in Global.neighbors_alive_list:
			folder_c_name_label_02.text = " "
			folder_c_photo_02.set_texture(no_photo)
			folder_c_id_label_02.text = " "
			folder_c_occupation_label_02.text = " "
...
func _set_info_folder_C_04()->void :
	...
	if Global.is_campaign:
		if "Alf_Cappuccin" not in Global.neighbors_alive_list:
			folder_c_name_label_01.text = " "
			folder_c_photo_01.set_texture(no_photo)
			folder_c_id_label_01.text = " "
			folder_c_occupation_label_01.text = " "
			folder_c_appearence_label_01.text = " "
		if "Rafttelyn_Cappuccin" not in Global.neighbors_alive_list:
			folder_c_name_label_02.text = " "
			folder_c_photo_02.set_texture(no_photo)
			folder_c_id_label_02.text = " "
			folder_c_occupation_label_02.text = " "
...
func _to_game_over()->void :
	if game_over.is_visible_in_tree():
		...
		await get_tree().create_timer(TWEEN_TIME * 2).timeout
	await get_tree().create_timer(TWEEN_TIME).timeout
	if Global.current_campaign_day != 6:
		Global.just_complete_level = true
